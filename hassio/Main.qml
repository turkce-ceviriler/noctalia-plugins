...
QtObject {
    ...
    function _populateEntities(allStates) {
        ...
        for (const state of allStates) {
            if (!pinned.includes(state.entity_id)) continue
            root.entities.append({
                ...
                color_temp: state.attributes.color_temp_kelvin
                            ? Math.round(1000000 / state.attributes.color_temp_kelvin)
                            : (state.attributes.color_temp ?? -1),
    ...
    function _handleStateChange(data) {
        ...
        for (let i = 0; i < root.entities.count; i++) {
            if (root.entities.get(i).entity_id !== entity_id) continue
            ...
            root.entities.setProperty(i, "color_temp",
                newState.attributes.color_temp_kelvin
                ? Math.round(1000000 / newState.attributes.color_temp_kelvin)
                : (newState.attributes.color_temp ?? -1))
    ...
    function _scheduleReconnect() {
        const delay = Math.min(
            root._reconnectBaseInterval * Math.pow(2, root._reconnectAttempts),
            root._reconnectMaxInterval
        )
    ...
    function callLightService(entity_id, brightness, color_temp) {
        const id = _nextId()
        const serviceData = { entity_id: entity_id }
        if (brightness >= 0) serviceData.brightness = brightness
        if (color_temp >= 0) {
    ...
    function _supportsColorMode(modes, targets) {
        if (!modes || !Array.isArray(modes)) return false
    ...
    property Timer _settingsDebounce: Timer {
        ...
        repeat: false
        onTriggered: {
            console.log("[HASS] Settings changed, reconnecting...")
            root.authFailed = false
            root._resetReconnect()
            _socket.active = false
            root.connected = false
